...
<script>
  ...
  export default{
      ...
      mounted(){
          this.init();
          // this.checkIndex = this.defaultAddressId;
      },
      computed:{
        //控制地址的显示个数
        addressListFilter(){
        ...
        defaultAddressId(){
          ...
          this.addressList.forEach((item, index)=>{
            if(item.isDefault == true){
              ...
              flag = {id:item.addressId, index:index};
            }
          });
          return flag;
        }
      ...
      methods:{
          init(){
              axios.get("/users/addressList").then((response)=>{
                  ...
                  //默认地址选中
                  this.selectedAddrId = this.defaultAddressId.id;
                  this.checkIndex = this.defaultAddressId.index;
              });
          },
          expand(){
              if(this.limit==3){
          ...
          delAddressConfirm(addressId){
            //删除确认框
            this.isMdShow = true;
            this.delAddrId = addressId;
          },
          delAddress(){
            //删除地址
            axios.post("/users/delAddress",{
              addressId:this.delAddrId
            }).then((response)=>{
                let res = response.data;
                if(res.status=="0"){
